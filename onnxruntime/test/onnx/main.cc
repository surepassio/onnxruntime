...
void usage() {
  printf(
      ...
      "\t-O Run ORT format model. Requires .ort file extension"
      "\n"
      "\t-h: help\n"
      "\n"
...
int real_main(int argc, char* argv[], Ort::Env& env) {
  ...
  {
    int ch;
    while ((ch = getopt(argc, argv, ORT_TSTR("Ac:hj:Mn:r:e:xvo:d:pO"))) != -1) {
      switch (ch) {
        case 'A':
